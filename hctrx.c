...
static int     waitRx            (int sfd, int baudrate);
...
static void    transmitStream    (FILE *input, int fh);
...
int  main (int argc, char *argv[])
{
    ...
    in_fh = stdin;
    setdefaults (&hcsetup);
    if ((i = getArgs (argc, argv, &hcsetup)) < 0)
        return 0;

    if (i > 0)
        mode = hcsetup.trx_mode;


    /* set to operation mode */
    hc12_pinSetup();
#ifndef _PC_HOST_
    gpiod_line_set_value (lineSET, 1);       /* low-active ! */
#endif
    ...
    if ((iFDtrx = openLine (hcsetup.pSerLine, hcsetup.baudrate, termDelay, &oldoptions)) == -1)
    {
...
printf ("opened ser.line %s, (fh=%d,bri=%d\n", pSerLine, iFDtrx, BaudRate);
    ...
    /* try to open an input file if a name was given */
    if ((mode == MODE_TX) && (pInFileName != NULL))
    {
        in_fh = fopen (pInFileName, "r");
        if (in_fh == NULL)
        {
    ...
    if (mode == MODE_TX)
        transmitStream (in_fh, iFDtrx);

    else
    {
        do
        {
            waitRx (iFDtrx, hcsetup.baudrate);
            usleep (5000);
        }
        while (!stopRx);
    }
...
//  printf ("terminating...\n");
    fflush (stdout);
    if ((in_fh != NULL) && (pInFileName != NULL))
        fclose (in_fh);
    close(iFDtrx);
    return EXIT_SUCCESS;
}
...
/* if the first cmd line parameter is an option
 * and contains 't'/'T', return Tx mode;
 */
static int  getMode (char *argstr)
{
    if (strlen (argstr) <= 2)
        if ((strchr (argstr, 't')) || (strchr (argstr, 'T')))
            return MODE_TX;
    return MODE_RX;
}
...
/* receive transmitted characters, and echo them immediately;
 * returns after a timeout of half a FIFO buffer transmission time 
 */
int  waitRx (int sfd, int baudrate)
{
    int     s_end, i, k, timeout;
    char    c;
    struct  pollfd fds[2];
...
static int  getArgs (int argc, char **argv, struct hc12Setup *pSetup)
{
    ...
    if (c == -1)
    {
        ...
        /* a filename argument present ? */
        if ((optind+1) <= argc)
        {
            pInFileName = argv[optind+1];
            args++;
        }
    }
...
static void  printHelp (char *appName)
{
    fprintf (stdout, "  hctrx - a tool to drive a HC12 transmitter\n\n");
    fprintf (stdout, "call as : hctrx [options] <serial_line> [<input_file>]\n");
    fprintf (stdout, "recognized options are :\n");
    fprintf (stdout, "    -t            = Tx / transmit mode (default is Rx)\n");
    ...
    fprintf (stdout, " <input_file> is an optional file that is sent via the HC12 transmitter\n");
    fprintf (stdout, " a file parameter is ignored in the default Rx mode\n");
    fprintf (stdout, " parameters must match current transmitter settings to work\n");
}
...
/* take character from the input stream, and write them
 * to the output handle (transmitter);
 * delay for several milliseonds after each 64 byte block
 */
static void  transmitStream (FILE *input, int fh)
{
    int   count = 0;
    ...
    while (!stopRx);  /* flag set by SIGNAL handler */
}